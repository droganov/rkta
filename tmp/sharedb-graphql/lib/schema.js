...
Schema.prototype.implementType = function (typeShape, typeShapeInput) {
	...
	if(typeShapeInput) { // на основе пользовательской формы
		// расширение типа служебными полями (если их нет)
...
Schema.prototype.updateQueryString = function(typeName, queryString, fields, updateQueryName) {
	try{
		var parsedQuery = parseQuery(queryString);

		if(updateQueryName) { // обновление имени запроса
...
function filterSelection (self, selection, fields) {
	...
	// приделываем служебные поля в запрос
