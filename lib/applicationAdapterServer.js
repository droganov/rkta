...
function mountApp( koaServer, name, mountPoint ){
  ...
  const pathToApp = ( isProduction ? "../back-end/build/" : "../front-end/applications/" ) + name
  const clientApp = require( pathToApp )
  const koaApp = koa()
  koaApp.use( function *( next ){
    const self = this
    const stats = require( "../back-end/build/stats" )
    ...
    const reduxStore = clientApp.redux.configureStore( initialState );
    ...
    const preRender = yield function( cb ){
      clientApp.preRender( routes, location, racerModel, reduxStore.getState(), ( err, redirectLocation, renderProps ) => {
